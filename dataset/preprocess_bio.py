    ...
    if os.path.isdir(filepath):
        for sub_filename in os.listdir(filepath):
            ...
            if os.path.isfile(file) and sub_filename.endswith('tsv'):
                new_file = os.path.join(filepath, sub_filename.replace('.tsv', '_bio.json'))
                with open(file, mode='rt', encoding='utf-8') as inp, open(new_file, mode='wt', encoding='utf-8') as outp:
                    ...
                    for idx, row in enumerate(reader):
                        ...
                        for i in range(0, len(tokens)-target_len+1):
                            ...
                            if sub_sent == sub_tokens:
                                ...
                                labels_list[i] = 'B-'+TAG
                                if target_len > 1:
                                    labels_list[i+1: i + target_len] = ['I-'+TAG] * (target_len-1)
                                break
                        if not SUCC:
                            err_count += 1
                            all_err_count += 1
